 ...
 * @vue-prop {string} [fileNameKey] 上传的文件名称key值
 * @vue-event {Object} input 修改文件事件(v-model)
 * @vue-event {Object} change 修改文件事件
 *  ps: 属性 允许上传文件类型 fileSuffixs 优先级大于 不允许上传文件类型 notAllowedFiles
...
export default {
  ...
  watch: {
    ...
    value(val) {
      if(this.isVideo && this.isReturnUrl) {
        if(val) {
          let obj = {}
          if(this.isOriginalData) {
            obj[this.fileUrlNameKey] = val
          } else {
            obj.url = val
          }
  ...
  methods: {  
    ...
    handleDownload(file) {
      ...
      this.downloadUrlFile(file[this.fileUrlNameKey], file[this.fileNameKey]);
    },
    //上传文件
    uploadFile(item) {
      if (this.httpRequest) {
      ...
      uploadFile(this.action, this.method, formData, onUploadProgress, this.needToken, this.timeout,this.dynamicConfig).then(response => {
        let data = response;
        if (this.resFormat) {
        ...
        const fileData = {
          url: _data[this.fileUrlNameKey] || fileUrl || url,
          name: _data[this.fileNameKey] || fileOriginalName || fileName,
          id: fileId,
          res: _data
        };
        const resData = this.isOriginalData ? _data : fileData
        if (this.isVideo && this.isReturnUrl) {
        ...
        //限制文件类型
          if (this.fileSuffixs != "all") {
            ...
            if (!includes) {
             ...
             if(this.notAllowedFiles.length){
                const {
                  name
                } = file;
                ...
                const suffixList = this.notAllowedFiles;
                if (suffixList.includes(suffix)) {
                  this.$message.error(`请您上传正确的文件类型。`);
                  return false;
                }
             }
          }
