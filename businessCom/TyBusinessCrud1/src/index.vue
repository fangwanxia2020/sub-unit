<template>
  <div class="ty-business-crud">
    ...
    >
            ...
            <div class="searchBox-form">
              <template v-if="showSearch">
                ...
                <div class="searchBox-btn">
                  ...
                  <template v-if="showSearch && searchMode == 'default'">
                    <span v-if="showOperation.indexOf('add') >= 0">
      ...
      <!-- 批量按钮工具栏 -->
     <div v-show="hasBatchBtn && selectedList.length" class="button-batch-group">
      <div class="batch-btn">
        <div class="batch-l">
          ...
          <slot name="button-batch-group">
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="(item,index) in BatchBtnList" :key="index">
                  <span @click="clickBatchOptions(item.type)">{{item.name}}</span>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </slot>
...
export default {
 ...
 //控制行的样式  ylq 2022/11/03
    rowClassNameControl: {
      type: Function,
    },
   //是否高亮表格当前行   ylq 2022/11/03
    highlightCurrentRow:{
       type: Boolean,
       default: false,
    },
  ...
  data() {
    ...
    return {
      ...
      showBtnList: false,
      sortList: [],
      outSiderQueryChange: this.outSiderQuery,
  ...
  watch: {
    ...
    outSiderQuery: {
      handler(newVal) {
        this.outSiderQueryChange = newVal
        this._getListData()
      },
    },
  },
  computed: {
    ...
    editFormData() {
      ...
      this.dialogTitle=this.columns.filter((item) => item.isDialogTitle == true)[0]?.prop
  ...
  methods: {
    ...
    _getListData() {
      ...
      postData = this.isGetListByQuery
        ? {
            [this.pageFormat['pageNum'] || 'pageNum']: pageNo,
            [this.pageFormat['pageSize'] || 'pageSize']: limit,
            query: {
            ...
            // orderBy: postData.sortList,
            ...this.outSiderQueryChange,
          }
        : postData
